        ...
        for ctx in contexts:
            if ctx[1]['Type'].value == "internet":
                if self.get_bearer_path_for_apn(ctx[1]['AccessPointName'].value) is not None:
                    continue
                self.create_bearer_interface(ctx[0], ctx[1])

    def ofono_context_added(self, path, properties):
        if properties['Type'] == "internet":
            self.create_bearer_interface(path, properties)

    def create_bearer_interface(self, path, properties):
        global bearer_i
        Logger.info(f"Creating new bearer interface for {path} with index {bearer_i}")
